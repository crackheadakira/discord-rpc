...

    {
...

        {
            ...
            if (presence != nullptr) {
                ...
                else if ((presence->buttonLabel[0] && presence->buttonUrl[0]) ||
                         (presence->buttonLabel[1] && presence->buttonUrl[1])) {
                    WriteArray buttons(writer, "buttons");

                    for (int i = 0; i < 2; i++) {
                        if (presence->buttonLabel[i] && presence->buttonUrl[i]) {
                            WriteObject button(writer);
                            WriteOptionalString(writer, "label", presence->buttonLabel[i]);
                            WriteOptionalString(writer, "url", presence->buttonUrl[i]);
                        }
                    }
                }
